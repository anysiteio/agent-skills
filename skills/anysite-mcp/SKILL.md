...
The anysite MCP exposes ~550 data sources through five universal meta-tools. This skill is the
map: how to call them, which sources cover which GTM need, and how to not waste credits.
...
| `get_page(cache_key, page)` | Page through a cached result | free |
| `query_cache(cache_key, conditions, aggregate, group_by)` | Filter/sort/aggregate cached data with SQL-like ops | free |
...
6. **Avoid `gdelt`** — it regularly exceeds 120s and times out. Use techmeme / google news
   / owler news instead.
...
- `linkedin/search/search_sql_companies` — the workhorse. Up to 1000 companies per call with
  DSL filters (keywords, industry_name, employee_count, country_hq, founded, has_website).
  Also does batch lookup by `urn` list and search by `website` — use it to resolve domains
  from a CRM into LinkedIn company records.
- `crunchbase/db/db_search` — filters by funding stage, last funding date, investors,
  employee range. 1 credit/result. Response includes `funding_rounds[]`, `leadership_hires[]`,
  `layoffs[]`, `news[]`, `technologies[]`, `employees[]`.
- `crunchbase/search` (live, 20cr/50) — adds `hiring`, `event`, `spotlight`,
  `shares_investors_with`, `it_spend_*`, `revenue_*`, `valuation_*` filters.
- `yc/search/companies`, `betalist`, `tracxn/companies/search` — early-stage supplements.

**Company detail:** `crunchbase/company` (by alias — resolve via `crunchbase/search` first),
`linkedin/company`, `owler/companies` (+ `/competitors`, `/news`).
...
2. `linkedin/user/find_email_by_url` — by vanity URL, ~75% yield, 50cr. Run only on the
   remainder after step 1.
3. No email found → keep the lead anyway; CRM upserts match by `linkedin_url` too.

**Reverse lookup (email → person):** `linkedin/email/email_sql_user` (cached DB) →
`linkedin/email/email_user` (live) for the remainder.

**Hiring signals:**
- `linkedin/search/search_jobs` — by company URN (resolve via `search_companies`); works for
  any company.
- `greenhouse/jobs/jobs_search`, `ashby/jobs` — by board token (company slug; 412 = token
  wrong, fall back to linkedin jobs). Full descriptions available (`content=true`).
- `glassdoor/companies` + `/jobs`, `builtin`, `adzuna` — supplements; `blind/layoffs/search`
  for layoffs.
...
**Software reviews:** `g2/products/products_search` (search only), `capterra/products/reviews`,
`trustradius/products/reviews`, `getapp/products/reviews`, `gartner/products` — competitor
review mining. Employer sentiment: `glassdoor/companies/ratings`, `kununu`, `comparably`,
`blind/companies/reviews`.

**News & mentions:** `techmeme/stories/stories_search` (archive search) and
`stories_front_page`; `google/news/articles/search`; `owler/companies/news`;
`linkedin/search/search_posts` (keyword or `mentioned` company URN, `date_posted` filter);
`reddit`, `hackernews`, `twitter`, `bluesky` for community chatter; `substack`/`medium` for
content signals.

**Launches & products:** `producthunt/launches/search`, `products/alternatives`,
`products/reviews`; `indiehackers`, `kickstarter`/`indiegogo` for niche ICPs.
